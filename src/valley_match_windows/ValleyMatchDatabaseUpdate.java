 ...
 * @version 2.1 --- Feb 9, 2016 -- Adapted to run on the main computer in B202
 * 								   with a Windows 7 environment
 * 
 */
		...
		if (openDialogResult == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				try {
					...
					dbConnection = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + DATABASE + "?useSSL=false", "valleymatch", "xxxx");
					...
					for (Row entrant: masterList) {
						...
						if (hasValidCode == Validity.VALID) {
							...
							ec.close();
							System.out.println("Sent Email to " + entrant.getCell(0).getStringCellValue() + " about " + Validity.NOT_FOUND_ERROR);
						} else if (hasValidCode == Validity.REUSE_ERROR) {
							// Send email to person notifying them that their access code was already used by someone else
							ec = new EmailClient(entrant.getCell(0).getStringCellValue());
							ec.sendCustomEmail("Invalid Access Code", "Dear Entrant,\n\nIt appears as though your access code was found in our database but"
									...
									+ " GVHS Robotics Team.\n\nThank you for your support.  Happy Matching!");
							ec.close();
							System.out.println("Sent Email to " + entrant.getCell(0).getStringCellValue() + " about " + Validity.REUSE_ERROR);
						} else {
							...
							ec.sendCustomEmail("Invalid Access Code", "Dear Entrant,\n\nSomething went wrong on our end.  Unfortunately, your information"
									...
									+ " database.\n\nThank you for your support.  Happy Matching!");
							ec.close();
							System.out.println("Sent Email to " + entrant.getCell(0).getStringCellValue() + " about " + Validity.UNKNOWN_ERROR);
						}

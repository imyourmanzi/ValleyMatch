 ...
 * 									removed certain variables for effciency,
...
//		final String STUDENT_TABLE_NAME = "ValleyMatchRobotics";	// LOCAL TEST INFO
		...
		try {
			...
			backupFile = new File("/Users/matteomanzi/Code/Java Workspace/ValleyMatch/backup/Results" + (new SimpleDateFormat("yyyyMMdd-HHmm").format(Calendar.getInstance().getTime())) + ".bak.csv");
...
//				System.out.println("Entered matcheeList");
...
//				System.out.println("Set up matchee " + crntMatchee.getName());
				
				while (matchedList.next()) {
					
//					System.out.println("Entered matchedList");
...
//						System.out.println("Matchee " + crntMatchee.getName() + " has " + crntMatched.getName() + " as eligible");
						
						for (int i = 0; i < TOTAL_FIELDS ; i++) {
							
//							System.out.println("Entered criteria for-loop");
...
//							System.out.println("Compared criterion, added to csv, added to crntMatched " + COLUMN_NAME[i]);
						} // End for()
						
//						System.out.println("Exited criteria for-loop");
...
//				System.out.println("Exited matchedList");
				
				matchedList.beforeFirst();
				allMatchees.add(crntMatchee);
			} // End while(matchedList.next())
			
//			System.out.println("Exited matcheeList");
...
//			System.out.println(matchee.toString());
//		}
	...
	private static CachedRowSet getAllDatabaseContentsForDatabaseAndTable(String database, String table) {
		...
		try {
			...
			Connection dbConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + database + "?useSSL=false", "myuser", "xxxx");
